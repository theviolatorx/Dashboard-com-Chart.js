...
def retorna_total_vendido(request):
    total = Venda.objects.all().aggregate(Sum('total'))['total__sum']
...
def relatorio_faturamento(request):
    ...
    for i in range(12):
        ...
        y = sum([i.total for i in x if i.data.month ==
                mes and i.data.year == ano])
...
def relatorio_produtos(request):
    ...
    for produto in produtos:
        vendas = Venda.objects.filter(
            nome_produto=produto).aggregate(Sum('total'))
...
def relatorio_funcionario(request):
    ...
    for vendedor in vendedores:
        vendas = Venda.objects.filter(
            nome_vendedor=vendedor).aggregate(Sum('total'))
        if not vendas['total__sum']:
            vendas['total__sum'] = 0
